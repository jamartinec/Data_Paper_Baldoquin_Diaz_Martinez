...
model = AbstractModel(name="m1_gulnara_version_mayo7")
...
model.y = Var(model.v_cst, model.P, within= Binary)# If client i is assigned to pattern p.

model.r=Var(model.v_cst, model.T, model.M, within=NonNegativeReals)

model.zmaxi = Var(within=NonNegativeReals)
...
model.R2_a = Constraint(model.subcl2,rule=R2_a)
...
#def R6a(model,i,l,k):
#    return model.ri[i,l,k] >= model.e[i]*model.x[1, i, l]
#model.R6= Constraint(model.v_cst,model.T, model.M, rule=R6a)


# def R6b(model, i, l,k):
#     return model.ri[i, l, k] >= model.e[i] * model.x[1, i, l]
# model.R6a = Constraint(model.v_cst,model.T, model.M, rule=R6b)
...
def R13(model, i,l,k):
    return model.r[i,l,k] >=  model.t[i,l]+ model.d[i,model.n+2]*model.x[i,model.n+2,l]-model.D*(1-model.x[i,model.n+2,l])-model.D*(1-model.w[i,k])
model.R13 = Constraint(model.v_cst, model.T, model.M, rule=R13)

def R13v2(model, i,l,k):
    return model.r[i,l,k] <= model.zmaxi
model.R13v2 = Constraint(model.v_cst,model.T, model.M,rule=R13v2)



#Symmetry Breaking (Eq 29 paper)
#def Rcorte2(model):
#	return model.w[model.n+1,1]==1
#model.Rcorte2= Constraint(rule=Rcorte2)

#At least 2 customers per route (Equation 28 paper)
#def R24(model,i,l):
#	return model.x[1,i,l] + model.x[i,model.n+2,l] <=1
#model.R24 = Constraint(model.v_cst, model.T,rule=R24)
...
# (Equation 30 paper)
#def Rcorte3(model,i,j,l):
#	if (i==j):
#		return Constraint.Skip
#	else:
#		return model.x[i,j,l]+ model.x[j,i,l] <=1
#model.Rcorte3 = Constraint(model.v,model.v,model.T,rule=Rcorte3)
